...
def html_convert():
    ...
    original_soup = BeautifulSoup(combined_page, features="lxml").find('main')
    ...
    return render_template(
        f"code_{lang}.html", lang=lang, html_page=html_page, aem_page=aem_page, total_score = total_score, fkpoints = fkpoints, final_fk_score = final_fk_score, hpoints = hpoints, hratio = hratio, ppoints = ppoints, pratio = pratio, total_words = total_words, url = url, word_column_names = word_column_names, row_data_word_en = list(mc_en.values.tolist()), row_data_word_fr = list(mc_fr.values.tolist()), zip = zip, score = score, len_headings = len_headings, len_par = len_par, original_score = original_score
    )
